...
export function CommentsSheet({
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        const rows = (data ?? []).map((r: { id: string; text: string; created_at: string; author_id: string; profiles: { name: string | null; photo_url: string | null } | null }) => ({
          id: r.id,
          text: r.text,
  ...
  async function send() {
    ...
    const { data, error } = await supabase
      ...
      .single();
    setSending(false);
    if (error || !data) { toast.error(error?.message ?? "Falha ao comentar"); return; }
    setComments((prev) => [...prev, { id: data.id, text: data.text, created_at: data.created_at, author_id: data.author_id, author: data.profiles }]);
    setText("");
    onCountChange?.(1);
  }
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            <div className="pt-2 pb-1 flex justify-center"><div className="h-1 w-10 rounded-full bg-border" /></div>
            <div className="px-4 pb-2 flex items-center">
              <h3 className="font-display font-bold text-base flex-1">Comentários</h3>
              <button onClick={onClose} className="h-8 w-8 grid place-items-center rounded-full bg-secondary"><X className="h-4 w-4" /></button>
            </div>
                ...
                <ul className="space-y-3">
                  {comments.map((c) => (
                    <li key={c.id} className="flex items-start gap-2.5">
                      <img
                        src={c.author?.photo_url ?? `https://api.dicebear.com/9.x/initials/svg?seed=${c.author?.name ?? "?"}`}
                        alt="" className="h-8 w-8 rounded-full object-cover"
                      />
                      <div className="flex-1 rounded-2xl bg-secondary px-3 py-2">
                        <div className="text-[11px] font-semibold text-primary">{c.author?.name ?? "Anônimo"}</div>
