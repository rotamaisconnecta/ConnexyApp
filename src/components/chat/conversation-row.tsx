...
export function ConversationRow({ conversation, onOpen, onGesture, onMenu }: ConversationRowProps) {
  const {
    participant,
    unreadCount,
    isMuted,
  ...
  return (
    ...
    >
      <button
        ...
        className="absolute inset-0 z-0 rounded-2xl outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
      />
      ...
      <div className="pointer-events-none min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            ...
            {isMuted && <BellOff className="h-3 w-3 text-muted-foreground/70" strokeWidth={2.2} />}
            <span
              className={cn(
      ...
      <div className="relative z-10 flex shrink-0 flex-col items-center gap-1.5">
        {gestureLabel && (
          <button
            ...
            className={cn(
              "pointer-events-auto whitespace-nowrap rounded-full bg-primary/10 px-3 py-1 text-[11px] font-semibold text-primary transition-all duration-200",
        ...
        <button
          type="button"
          onClick={() => onMenu(conversation)}
          aria-label={`Mais opções para ${participant.name}`}
          className="pointer-events-auto grid h-8 w-8 place-items-center rounded-full text-muted-foreground transition-colors hover:bg-secondary active:bg-accent"
        >
