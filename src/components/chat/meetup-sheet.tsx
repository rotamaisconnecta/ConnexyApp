...
export function MeetupSheet({
  open, onClose, personName, onSuggest, onShareMyLocation,
}: {
  open: boolean;
  onClose: () => void;
  personName: string;
  ...
  const [pending, setPending] = useState<
    | { kind: "place"; pick: MeetupPick }
    | { kind: "self" }
    | null
  >(null);

  const confirmMessage = pending?.kind === "place"
                  ...
                  <div key={pl.id} className="rounded-2xl border border-border bg-surface p-3 flex items-center gap-3 shadow-soft">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <button
                        onClick={() => setPending({
                          kind: "place",
                          pick: { placeName: pl.name, cover: pl.cover, proximity: `${proximityLabel(pl.distanceMeters)} · ${proximityRadius(pl.distanceMeters)}` },
                        })}
