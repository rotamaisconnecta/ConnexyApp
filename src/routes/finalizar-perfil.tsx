...
function FinishProfile() {
  ...
  function addPhoto(index: number, file?: File) {
    if (!file) return;
    setPhotos((current) => current.map((photo, photoIndex) => photoIndex === index ? URL.createObjectURL(file) : photo));
  }
        ...
        <div className="px-6 pb-8 pt-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2" aria-label="Passo 4 de 4">
              {[0, 1, 2, 3].map((step) => <span key={step} className={`h-2 w-10 rounded-full ${step === 3 ? "bg-gradient-brand" : "bg-violet-200"}`} />)}
            </div>
            <span className="w-11" />
          </div>
          ...
          <header className="mt-7 text-center">
            ...
            <h1 className="mt-2 font-display text-[34px] font-semibold leading-tight">Quase pronto! <span aria-hidden>✦</span></h1>
          ...
          <section className="mt-9">
            ...
            <p className="mt-1 text-base text-muted-foreground">Mostre quem voce e! Adicione pelo menos 1 foto.</p>
            <div className="mt-5 grid grid-cols-3 gap-3">
              {photos.map((photo, index) => (
                ...
                >
                  {photo ? (
                    <img src={photo} alt={`Foto ${index + 1} do perfil`} className="h-full w-full object-cover" />
                  ) : index === 0 ? (
          ...
          <section className="mt-8">
            ...
            <div className="mt-3 flex h-14 items-center gap-3 rounded-2xl border border-violet-100 px-4 shadow-sm">
              ...
              <button type="button" className="font-medium text-violet-600">Alterar</button>
            </div>
          </section>
          ...
          <section className="mt-8">
            <h2 className="text-xl font-semibold">Verificacao de perfil <span className="font-normal text-muted-foreground">(opcional)</span></h2>
            <div className="mt-3 flex items-center gap-3 rounded-2xl border border-violet-100 p-3 shadow-sm">
              ...
              <p className="min-w-0 flex-1 text-sm leading-5 text-muted-foreground">Verifique seu perfil e ganhe mais confianca da comunidade.</p>
              <button type="button" className="shrink-0 rounded-xl border border-violet-500 px-3 py-2 text-sm font-medium text-violet-600">Verificar agora</button>
            </div>
          </section>
          ...
          <Link to="/interesses" className="mt-6 block text-center text-lg font-medium text-violet-600">Voltar</Link>
        </div>
      </div>
    </PhoneFrame>
  );
