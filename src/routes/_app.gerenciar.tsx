...
import { ChevronLeft, Loader2, Plus, Trash2, Image as ImageIcon, Video, MapPin, Save, LogOut, User as UserIcon, FileText, ExternalLink } from "lucide-react";
  ...
  return (
    <div className="flex-1 flex flex-col pb-24">
      ...
      <header className="px-4 pt-1 pb-3 flex items-center gap-2">
        <Link to="/perfil" className="h-9 w-9 grid place-items-center rounded-full bg-secondary"><ChevronLeft className="h-4 w-4" /></Link>
        <div className="flex-1">
          <h1 className="font-display font-bold text-lg">Gerenciar sua bio</h1>
          <p className="text-[11px] text-muted-foreground">Isto é o que as pessoas verão sobre você</p>
        </div>
        <button
          onClick={async () => {
...
function PerfilTab({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    supabase.from("profiles").select("*").eq("id", userId).maybeSingle().then(({ data }) => {
      const prof = (data as Profile | null) ?? {
        id: userId, handle: null, name: null, age: null, photo_url: null, bio: null,
        headline: null, mood_emoji: null, mood_text: null,
        interests: [], vibe_tags: [], looks_for: [],
      };
      setP(prof);
  ...
  if (!p) return <div className="p-6 grid place-items-center"><Loader2 className="h-5 w-5 animate-spin text-primary" /></div>;
  ...
  return (
    <div className="px-4 mt-4 space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <img
              ...
              alt="" className="h-16 w-16 rounded-2xl object-cover ring-2 ring-primary/20"
            />
      ...
      <Section title="Sobre você">
        <Field label="Frase de capa" placeholder="Ex.: Vinil, café e caminhadas sem destino" v={p.headline ?? ""} onChange={(v) => setP({ ...p, headline: v })} />
        <TextArea label="Bio" v={p.bio ?? ""} onChange={(v) => setP({ ...p, bio: v })} placeholder="Conte um pouco de você…" />
        <div className="grid grid-cols-2 gap-2">
          <Field label="Humor (emoji)" placeholder="☕" v={p.mood_emoji ?? ""} onChange={(v) => setP({ ...p, mood_emoji: v })} />
          <Field label="Humor (texto)" placeholder="modo café" v={p.mood_text ?? ""} onChange={(v) => setP({ ...p, mood_text: v })} />
        </div>
        <Field label="Idade" type="number" v={p.age?.toString() ?? ""} onChange={(v) => setP({ ...p, age: v ? Number(v) : null })} />
      </Section>

      <Section title="Tags (separadas por vírgula)">
        <Field label="Interesses" v={interestsText} onChange={setInterestsText} placeholder="Café, Música, Arte…" />
        <Field label="Vibe" v={vibeText} onChange={setVibeText} placeholder="cinéfilo, corujão…" />
        <Field label="Aqui buscando" v={looksText} onChange={setLooksText} placeholder="amizade, network…" />
      </Section>
      ...
      <Link to="/perfil/$id" params={{ id: userId }} className="block text-center text-sm text-primary font-semibold inline-flex items-center justify-center gap-1 w-full">
...
function PostsTab({ userId }: { userId: string }) {
  ...
  const reload = async () => {
    ...
    const [{ data: posts }, { data: pls }] = await Promise.all([
      supabase.from("bio_posts").select("*").eq("author_id", userId).order("created_at", { ascending: false }),
  ...
  async function create() {
    if (!newText.trim()) { toast.error("Escreva algo primeiro"); return; }
    const { error } = await supabase.from("bio_posts").insert({
      author_id: userId, text: newText.trim(),
      media_url: newMedia?.url ?? null, media_kind: newMedia?.kind ?? null,
      place_id: newPlaceId || null,
    });
    if (error) { toast.error(error.message); return; }
    toast.success("Post publicado!");
    setNewText(""); setNewMedia(null); setNewPlaceId(""); setCreating(false);
    reload();
  }
    ...
    <div className="px-4 mt-4 space-y-3">
          ...
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            ...
            {newMedia && (
              <div className="relative rounded-2xl overflow-hidden">
                ...
                <button onClick={() => setNewMedia(null)} className="absolute top-2 right-2 h-7 w-7 grid place-items-center rounded-full bg-foreground/60 text-white">
            ...
            <button
              onClick={create} disabled={uploading}
            ...
            >
              {uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Publicar
            </button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {loading ? (
        ...
        <div className="text-center py-10 rounded-3xl border border-dashed border-border">
          ...
          <p className="text-xs text-muted-foreground mt-1">Publique algo — vai aparecer na sua bio pública.</p>
        </div>
      ) : (
        <ul className="space-y-2.5">
          {posts.map((post) => {
            ...
            return (
              <li key={post.id} className="rounded-2xl border border-border bg-surface overflow-hidden shadow-soft">
                {post.media_url && post.media_kind === "image" && <img src={post.media_url} alt="" className="w-full h-40 object-cover" />}
                {post.media_url && post.media_kind === "video" && <video src={post.media_url} controls className="w-full h-40 object-cover" />}
                <div className="p-3">
                  <p className="text-sm">{post.text}</p>
                  <div className="mt-2 flex items-center justify-between text-[11px] text-muted-foreground">
                    <div>
                      {pl && <span className="text-primary font-semibold inline-flex items-center gap-1"><MapPin className="h-3 w-3" />{pl.name}</span>}
                      <span className="ml-2">{new Date(post.created_at).toLocaleDateString("pt-BR")}</span>
                    </div>
                    <button onClick={() => remove(post.id)} className="text-destructive inline-flex items-center gap-1"><Trash2 className="h-3.5 w-3.5" /> Apagar</button>
                  </div>
                </div>
              </li>
            );
...
function Field({ label, v, onChange, placeholder, type = "text" }: { label: string; v: string; onChange: (v: string) => void; placeholder?: string; type?: string }) {
  return (
    <label className="block">
      ...
      <input value={v} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} type={type}
